...
function runLibrarySystem() {
  // 1. CHỨC NĂNG BẢO MẬT (Sử dụng vòng lặp while)
  let loginAttempts = 0;
  ...
  while (loginAttempts < 3) {
    ...
    if (username === "admin" && password === "12345") {
      ...
      break; // Thoát khỏi vòng lặp đăng nhập
    } else {
      loginAttempts++;
  ...
  // 2. MENU CHỨC NĂNG (Sử dụng vòng lặp do-while)
  let choice;
  do {
...
// Chức năng 1: Phân loại mã số sách
function classifyBookIDs() {
  let totalBooks = 0;
  let evenBooks = 0; // Sách khoa học
  let oddBooks = 0; // Sách nghệ thuật
  let bookId;
...
// Chức năng 2: Thiết kế sơ đồ kho (Nested Loop)
function designWarehouseMap() {
  ...
  for (let i = 1; i <= rows; i++) {
    ...
    console.log(rowLayout); // In từng hàng ra màn hình console
  }
  alert("Đã in bản đồ kho ra Console (F12).");
}

// Chức năng 3: Dự toán phí bảo trì
